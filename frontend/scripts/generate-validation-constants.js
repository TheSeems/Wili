...
function extractValidationConstants(spec) {
    ...
    if (itemDataSchema?.properties) {
        ...
        if (itemDataSchema.properties.description) {
            constants.ITEM_DESCRIPTION_MAX_LENGTH = itemDataSchema.properties.description.maxLength || 2000;
        }
    }
    ...
    if (createWishlistSchema?.properties) {
        ...
        if (createWishlistSchema.properties.description) {
            constants.WISHLIST_DESCRIPTION_MAX_LENGTH = createWishlistSchema.properties.description.maxLength || 2000;
        }
    }
...
`;
    ...
    entries.forEach(([key, value]) => {
        const description = key.toLowerCase()
            .replace(/_/g, ' ')
