<script lang="ts">
	...
	async function loadWishlists() {
		...
		try {
			...
			error =
				err instanceof Error ? err.message : $_('wishlists.failedToLoad');
	...
	<div class="flex items-center justify-between mb-8">
		<div>
			...
			<p class="text-muted-foreground mt-2">
				<T key="wishlists.manageDescription" fallback="Manage your wishlists and track your desired items" />
			</p>
		</div>
		...
		<div class="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
			{#each Array(6) as _}
				<Card>
					<CardHeader>
						<div
							class="h-4 bg-gray-200 dark:bg-black rounded animate-pulse"
						></div>
						<div
							class="h-3 bg-gray-100 dark:bg-black rounded animate-pulse mt-2"
						></div>
					</CardHeader>
					<CardContent>
						<div
							class="h-3 bg-gray-100 dark:bg-black rounded animate-pulse"
						></div>
					</CardContent>
				</Card>
			{/each}
					...
					<CardContent>
						...
						>
							<div class="flex items-center gap-1">
								...
								<span>{wishlist.items?.length || 0} <T key="wishlists.items" fallback="items" /></span>
							</div>
							<div class="flex items-center gap-1">
