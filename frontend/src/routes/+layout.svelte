<script lang="ts">
  ...
  const pageTitle = $derived(page.url.pathname === '/privacy' ? 'Privacy Policy — Wili' : defaultTitle);
</script>
  ...
  {#if page.url.pathname !== "/auth/callback"}
    <footer class="mt-10 py-6 text-center text-sm text-gray-500 dark:text-gray-400">
      <div class="flex items-center justify-center gap-4 opacity-70">
        ...
        >
          <svg class="h-4 w-4" viewBox="0 0 98 96" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
        ...
        <a href="/privacy" class="hover:text-gray-700 dark:hover:text-gray-300"><T key="footer.privacy" fallback="Privacy Policy" /></a>
      </div>
    </footer>
  {/if}
{:else}
