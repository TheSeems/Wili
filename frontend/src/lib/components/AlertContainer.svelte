<script lang="ts">
	...
	$: alertsByPosition = alerts.reduce((acc, alert) => {
...
{#each Object.entries(alertsByPosition) as [position, positionAlerts]}
	<div class="{getPositionClasses(position)} flex flex-col gap-2 max-w-md w-full pointer-events-none">
			...
			>
				<Alert.Root variant={alert.variant || 'default'} class="pr-8">
					...
					<button
						...
						class="absolute top-2 right-2 p-1 rounded-sm opacity-70 hover:opacity-100 transition-opacity"
					>
						<XIcon class="h-3 w-3" />
					</button>
				</Alert.Root>
